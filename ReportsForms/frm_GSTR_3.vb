...
Public Class frm_GSTR_3
    ...
    Private Function AddBasicData(xlWorkSheet As Excel.Worksheet) As Excel.Worksheet
        ...
        For Each ch As Char In row("TinNo_vch").ToString
            xlWorkSheet.Cells(6, colIndex) = ch.ToString
            colIndex += 1
        Next
        xlWorkSheet.Cells(7, 2) = row("OutletName_vch")
        Return xlWorkSheet
    End Function

    Private Function Add3_1SectionData(xlWorkSheet As Excel.Worksheet) As Excel.Worksheet
        row = Get3_1_SectionData(" And LEN(am.VAT_NO) > 0 AND (invd.VAT_PER)>0")
        ...
        row = Get3_1_SectionData(" And LEN(am.VAT_NO) = 0 ")
    ...
    Private Function Get5SectionData() As DataRow
        Qry = " SELECT " &
             " SUM(CASE WHEN sm.STATE_ID = " & stateId & " THEN ((BAL_ITEM_QTY * BAL_ITEM_RATE) - ISNULL(ITEM_DISCOUNT,0)) ELSE 0 END) AS IntraState_TaxableValue, " &
             " SUM(CASE WHEN sm.STATE_ID != " & stateId & " THEN ((BAL_ITEM_QTY * BAL_ITEM_RATE) - ISNULL(ITEM_DISCOUNT,0)) ELSE 0 END) AS InterState_TaxableValue " &
    ...
    Private Function Get4_SectionDataForPurchase() As DataRow
        Qry = " SELECT SUM(ISNULL(non_integrated_tax,0)) AS non_integrated_tax, " &
            " SUM(ISNULL(integrated_tax,0)) As integrated_tax FROM (" &
            " SELECT  SUM(CASE WHEN cm.STATE_ID = " & stateId & " THEN mrn.GST_AMOUNT ELSE 0 END) AS non_integrated_tax," &
            "         SUM(CASE WHEN cm.STATE_ID != " & stateId & " THEN mrn.GST_AMOUNT ELSE 0 END) AS integrated_tax " &
    ...
    Private Function Get3_1_SectionData(condition As String) As DataRow
        Qry = " SELECT " &
             ...
             " isnull(SUM(CASE WHEN sm.STATE_ID = " & stateId & " THEN invd.VAT_AMOUNT ELSE 0 END),0) AS non_integrated_tax, " &
             " isnull(SUM(CASE WHEN sm.STATE_ID != " & stateId & " THEN invd.VAT_AMOUNT ELSE 0 END),0) AS integrated_tax " &
             " FROM    dbo.SALE_INVOICE_MASTER inv " &
             " INNER JOIN dbo.SALE_INVOICE_DETAIL invd ON invd.SI_ID = inv.SI_ID" &
             " INNER JOIN dbo.ACCOUNT_MASTER am ON am.ACC_ID = inv.CUST_ID " &
             " INNER JOIN dbo.CITY_MASTER cm ON cm.CITY_ID = am.CITY_ID " &
    ...
    Private Function GetBasicData() As DataRow
        Qry = "SELECT OutletName_vch, TinNo_vch FROM dbo.OutletMaster"
        Return objCommFunction.Fill_DataSet(Qry).Tables(0).Rows(0)
    End Function
