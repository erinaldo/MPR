...
Public Class frm_Sale_Analysis
    Implements IForm
    ...
    Private Sub BindData()
        ...
        Query = "SELECT ROW_NUMBER() OVER (ORDER BY [SI_ID]) as SrNo,convert(varchar(20), OrderDate,106) as [Bill Date], BillNo as [Bill No.], BillAmount as [Bill Amount], ACC_NAME as [Account Name], ADDRESS_PRIM as [Address], VAT_NO as [GST No.], Mobile_no as [Mobile No.], BrandName as [Brand], ITEM_CAT_NAME as [Category], Item_Name as [Item], BarCode_vch as [BAR Code], HsnCode_vch as [HSN Code],Cast(MRP_num as Numeric(18,2)) as [MRP], Cast(Price_num as Numeric(18,2)) AS [Base Price], Cast(ITEM_QTY as Numeric(18,2)) as [Quantity] ,um_name as [UOM], Cast(Price_num as Numeric(18,2)) as [Sale Rate], Cast(GrossAmount as Numeric(18,2)) AS [Taxable Value], TaxPercentage_num as [GST%], Cast(VAT_AMOUNT as Numeric(18,2)) as [Tax], CessPercentage_num as [CESS%], Cast(CessAmount_num as Numeric(18,2)) AS [CESS], Cast(ACessAmount as Numeric(18,2)) AS [ACESS], Cast(ItemTotalAmount_num as Numeric(18,2)) as [Total], SizeName as [Size], ColorName as [Color],CompanyName as [Make], DepartmentName as [Department], TypeName as [Type]  FROM Sales_Analysis WHERE CAST(OrderDate AS DATE)BETWEEN CAST('" & dtpFromDate.Value & "' AS DATE) AND CAST('" & dtpToDate.Value & "' AS DATE)  "
        ...
        If CategoryIds <> "" Then
            ...
            If CategoryIds.Length > 0 Then
                Filter = Filter & " AND ITEM_CAT_ID IN(" & CategoryIds & ") "
            End If
        ...
        If Not String.IsNullOrEmpty(txtSaleRateFrom.Text) Then
            Filter = Filter & " AND Price_num >=" & txtSaleRateFrom.Text
        End If

        If Not String.IsNullOrEmpty(txtSaleRateTo.Text) Then
            Filter = Filter & " AND Price_num <=" & txtSaleRateTo.Text
        End If


        If Not String.IsNullOrEmpty(txtBasePriceFrom.Text) Then
            Filter = Filter & " AND Price_num >=" & txtBasePriceFrom.Text
        End If

        If Not String.IsNullOrEmpty(txtBasePriceTo.Text) Then
            Filter = Filter & " AND Price_num <=" & txtBasePriceTo.Text
        End If
        ...
        If cmbGST.SelectedIndex > 0 Then
            Filter = Filter & " AND TaxID_num in ( " & cmbGST.SelectedValue & " )"
        End If
        ...
        If Not String.IsNullOrEmpty(txtBillNo.Text) Then
            Filter = Filter & " AND BillNo = '" & txtBillNo.Text & "'"
        End If
        ...
        Query = Query & " Order By SI_ID"

        If rbItemWise.Checked = True Then
            Query = "SELECT ROW_NUMBER() OVER (ORDER BY [ITEM_NAME]) as SrNo,
        ITEM_NAME AS [Item],
        um_name AS [UOM],
        Cast(SaleQty as Numeric(18,2)) AS [Quantity],
        Cast(GrossAmount as Numeric(18,2)) AS [Gross Amount],
        Cast(( GrossAmount / SaleQty ) AS Numeric(18,2)) AS [AvgPrice] ,
        Cast(GstAmount as Numeric(18,2)) AS [Tax],
        Cast(CessAmt as Numeric(18,2)) AS [CESS],
        Cast(ACessAmt as Numeric(18,2)) AS [ACESS],
        Cast(TotalBill as Numeric(18,0)) AS [Total Bills]
        FROM    ( SELECT    ITEM_NAME ,
                    ITEM_ID ,
                    um_name ,
                    ISNULL(SUM(ITEM_QTY), 0) AS SaleQty ,
                    ISNULL(SUM(GrossAmount), 0) AS GrossAmount ,
                    ISNULL(SUM(VAT_AMOUNT), 0) AS GstAmount ,
                    ISNULL(SUM(CessAmount_num), 0) AS CessAmt ,
                    ISNULL(SUM(ACessAmount), 0) AS ACessAmt ,
                    COUNT(distinct BillNo) AS TotalBill
          FROM      dbo.Sales_Analysis
          WHERE     CAST(OrderDate AS DATE) BETWEEN CAST('" & dtpFromDate.Value & "' AS DATE)
                                               AND     CAST('" & dtpToDate.Value & "' AS DATE)" & Filter &
          "  Group BY  ITEM_ID ,
                    ITEM_NAME ,
                    um_name
        ) tb"
        ...
        FROM    ( SELECT    ACC_NAME ,
                    ...
                    ISNULL(SUM(GrossAmount), 0) AS GrossAmount ,
                    ISNULL(SUM(VAT_AMOUNT), 0) AS GstAmount ,
                    ISNULL(SUM(CessAmount_num), 0) AS CessAmount ,
                    ISNULL(SUM(ACessAmount), 0) AS ACessAmt ,
                    COUNT(Distinct BillNo) AS TotalBill
          FROM      dbo.Sales_Analysis
          WHERE     CAST(OrderDate AS DATE) BETWEEN CAST('" & dtpFromDate.Value & "' AS DATE)
        ...
        If rbDall.Checked = True Then
            ...
            dgvReport.Columns(4).Width = 220
            dgvReport.Columns(6).Width = 115
            dgvReport.Columns(7).Width = 80
            dgvReport.Columns(8).Width = 200
            dgvReport.Columns(9).Width = 200
            dgvReport.Columns(10).Width = 280
            dgvReport.Columns(12).Width = 80
            dgvReport.Columns(13).Width = 80
            dgvReport.Columns(14).Width = 80
            dgvReport.Columns(15).Width = 80
            dgvReport.Columns(16).Width = 40
            dgvReport.Columns(17).Width = 80
        ...
        Else
            ...
            If rbAccWise.Checked = True Then
                ...
                dgvReport.Columns(4).Width = 65
                dgvReport.Columns(5).Width = 80
                dgvReport.Columns(6).Width = 70
            ...
            If rbItemWise.Checked = True Then
                ...
                dgvReport.Columns(0).Width = 50
                dgvReport.Columns(1).Width = 220
                dgvReport.Columns(2).Width = 40
    ...
    Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged
        ...
        Else
            If rbAccWise.Checked = True Then
                TryCast(dgvReport.DataSource, DataTable).DefaultView.RowFilter = String.Format("[Account Name] LIKE '{0}%'", txtSearch.Text)
            End If
            If rbItemWise.Checked = True Then
