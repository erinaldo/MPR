...
Public Class frm_GeneralLedger
    ...
    Public Sub CustomerBind()
        clsObj.ComboBind(cmbSupplier, "Select ACC_ID,ACC_NAME from ACCOUNT_MASTER WHERE AG_ID not in (" & AccountGroups.Sundry_Creditors & "," & AccountGroups.Sundry_Debtors & ") Order by ACC_NAME", "ACC_NAME", "ACC_ID", True)
    End Sub
    Public Sub ReportOS()
        Dim filepath As String = ""
        Dim rep As New ReportDocument()
        filepath = ReportFilePath & "CryGeneralLedger.rpt"

        rep.Load(filepath)

        If cmbSupplier.SelectedValue = 0 Then
            rep.SetParameterValue("@accountId", DBNull.Value)
        Else
            rep.SetParameterValue("@accountId", cmbSupplier.SelectedValue)
        End If

        rep.SetParameterValue("@FromDate", txtFromDate.Value.Date)
        rep.SetParameterValue("@ToDate", txtTodate.Value.Date)
    ...
    Private Sub frm_GeneralLedger_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        CustomerBind()
    End Sub
