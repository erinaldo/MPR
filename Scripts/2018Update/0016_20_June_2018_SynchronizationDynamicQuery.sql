...
AS  
  ...
  -----------Insert Data into MMSPLUS from ADMINPLUS based on selected outlet-----------    
        ...
        INSERT  INTO [' + @destDB + '].dbo.COST_CENTER_MASTER  
                SELECT  *  
                FROM    [' + @sourceDB + '].dbo.COST_CENTER_MASTER 
                where [' + @sourceDB + '].dbo.COST_CENTER_MASTER.Division_Id = ' + @OutletId + '   
        ...
        INSERT  INTO [' + @destDB + '].dbo.USER_MASTER  
                SELECT  *  
                FROM    [' + @sourceDB + '].dbo.USER_MASTER_MMS where 
                [' + @sourceDB + '].dbo.USER_MASTER_MMS.division_id = ' + @OutletId + '  
		...
		EXEC sys.[sp_executesql] @insertDynamicQuery1  
          ...
          IF @NewOutlet = '1'  
            BEGIN  
                ...
                SELECT  *  
                FROM    [' + @sourceDB + '].dbo.CN_SERIES where [' + @sourceDB + '].dbo.CN_SERIES.DIV_ID = ' + @OutletId + '  
                  
                INSERT  INTO [' + @destDB + '].dbo.MRN_SERIES  
                SELECT  *  
                FROM    [' + @sourceDB + '].dbo.MRN_SERIES where [' + @sourceDB + '].dbo.MRN_SERIES.DIV_ID = ' + @OutletId + '  
                  
                INSERT  INTO [' + @destDB + '].dbo.INVOICE_SERIES  
                SELECT  *  
                FROM    [' + @sourceDB + '].dbo.INVOICE_SERIES where [' + @sourceDB + '].dbo.INVOICE_SERIES.DIV_ID = ' + @OutletId + '  
                  
                INSERT  INTO [' + @destDB + '].dbo.DN_SERIES  
                SELECT  *  
                FROM    [' + @sourceDB + '].dbo.DN_SERIES where [' + @sourceDB + '].dbo.DN_SERIES.DIV_ID = ' + @OutletId + '  
                  
                INSERT  INTO [' + @destDB + '].dbo.DC_SERIES  
                SELECT  *  
                FROM    [' + @sourceDB + '].dbo.DC_SERIES where [' + @sourceDB + '].dbo.DC_SERIES.DIV_ID = ' + @OutletId + '  
                  
                INSERT  INTO [' + @destDB + '].dbo.PM_Series  
                SELECT  *  
                FROM    [' + @sourceDB + '].dbo.PM_Series where [' + @sourceDB + '].dbo.PM_Series.DIV_ID = ' + @OutletId + '  
                  
                INSERT  INTO [' + @destDB + '].dbo.PO_SERIES  
                SELECT  *  
                FROM    [' + @sourceDB + '].dbo.PO_SERIES where [' + @sourceDB + '].dbo.PO_SERIES.DIV_ID = ' + @OutletId + '' 
