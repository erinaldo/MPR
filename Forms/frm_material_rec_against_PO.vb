...
Public Class frm_material_rec_against_PO
    ...
    Private Sub bind_FLXGRD_PO_Items(ByVal po_id As String)
        ...
        lblSupplier.Text = Convert.ToString(obj.ExecuteScalar("select LTRIM(ACC_NAME +'  '+ CASE WHEN AG_ID=1 THEN 'Dr ' ELSE CASE WHEN AG_ID=2 THEN 'Cr ' ELSE '' END END +'  '+ ISNULL(VAT_NO,'')) AS ACC_NAME from ACCOUNT_MASTER  WHERE Acc_ID = (select po_supp_id from po_master where po_id = " + po_id + ")"))
        ...
        For iRow = 1 To FLXGRD_PO_Items.Rows.Count - 1
            ...
            If chk_ApplyTax.Checked = True Then
                ...
                Else
                    ...
                    FreightTaxAmount = Math.Round(Tax - ((totalAmount * FLXGRD_PO_Items.Item(iRow, "vat_per") / 2) / 100), 2)
            ...
            Else
                ...
                If lblMRNType.Text = "2" Then
                    ...
                    FreightTaxAmount = 0
                Else
                    Tax = Math.Round(totalAmount * (FLXGRD_PO_Items.Item(iRow, "vat_per") / 2) / 100, 2)
                    Tax = Math.Round((Tax * 2), 2)
                    FreightTaxAmount = 0
                End If
