...
Imports System.Text
Imports MMSPlus.frm_Customer_Rate_List_Master

Public Class frm_Customer_Rate_List_Master
    ...
    Private Sub frm_Supplier_Rate_List_Master_load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            chkActive.CheckState = CheckState.Indeterminate
            comboBind()
            flag = "save"
            'obj.FormatGrid(grdSupplierList)
            FillGrid()
            obj.ComboBind(cmbSupplier, "select 0 as ACC_ID,'--Select--' as ACC_NAME union Select ACC_ID,ACC_NAME from ACCOUNT_MASTER WHERE Is_Active=1 And AG_ID in (1,2,3,6) Order by ACC_NAME ", "ACC_NAME", "ACC_ID")
            grid_style()
        ...
        If grdSupplier.Rows.Count >= 1 And Not grdSupplier.Rows(0).Cells(3).Value = Trim("") Then
            ...
            Try
                ...
                If flag = "save" Then
                    ...
                    Dim Query As String = String.Format("exec UpdateCustomerRateListMapping {0},{1}", cmbSupplier.SelectedValue, int_MaxSRL_Id)
                    clsObj.ExecuteNonQuery(Query)
                ...
                Else
                    ...
                    Dim Query As String = String.Format("exec UpdateCustomerRateListMapping {0},{1}", cmbSupplier.SelectedValue, SRL_ID)
    ...
    Public Sub ViewClick(ByVal sender As Object, ByVal e As System.EventArgs) Implements IForm.ViewClick
        ...
        Try
            If grdSupplier.Rows.Count > 0 Then
                obj.RptShow(enmReportName.RptCustomerRateList, "SRL_ID", CStr(grdSupplierList("SRL_ID", grdSupplierList.CurrentCell.RowIndex).Value()), CStr(enmDataType.D_int))
            Else
    ...
    Private Sub FillGrid()
        Try
            ...
            grdSupplierList.Columns(1).HeaderText = "Rate List"
            grdSupplierList.Columns(2).HeaderText = "Description"
            grdSupplierList.Columns(3).Width = 140
            grdSupplierList.Columns(3).HeaderText = "Is Active"
            grdSupplierList.Columns(4).HeaderText = "Creation Date"
            grdSupplierList.Columns(5).Width = 300
            grdSupplierList.Columns(5).HeaderText = "Customer Name"
    ...
    Private Sub BindGrid()
        ...
        If chkActive.CheckState <> CheckState.Indeterminate Then
            query += " and active = '" & chkActive.Checked & "'"
        End If

        If Not String.IsNullOrEmpty(txtSearch.Text.Trim) Then
            query += " and (dbo.SUPPLIER_RATE_LIST.SRL_NAME + CONVERT(VARCHAR,SUPPLIER_RATE_LIST.SRL_DATE,101) + dbo.SUPPLIER_RATE_LIST.SRL_DESC + dbo.ACCOUNT_MASTER.ACC_NAME) LIKE '%" & Trim(txtSearch.Text) & "%'"
        End If

        obj.GridBind(grdSupplierList, query)
    End Sub
    ...
    Private Sub btn_ratelist_mapping_Click(sender As Object, e As EventArgs) Handles btn_ratelist_mapping.Click
        Dim frm_RateListMapping As New frm_RateList_Mapping()
        frm_RateListMapping.ShowDialog()
        FillGrid()
    End Sub

    Private Sub chkActive_CheckStateChanged(sender As Object, e As EventArgs) Handles chkActive.CheckStateChanged
        BindGrid()
    End Sub
